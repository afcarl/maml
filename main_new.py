...
def train(model, saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    for itr in range(resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations):
        ...
        if (itr % SUMMARY_INTERVAL == 0 or itr % PRINT_INTERVAL == 0):
            input_tensors.extend([model.summ_op, model.total_loss1, model.total_losses2, model.gating_distribution])
        ...
        if (itr!=0) and itr % TEST_PRINT_INTERVAL == 0 and FLAGS.datasource !='sinusoid':
            if 'generate' not in dir(data_generator):
                ...
                if model.classification:
                    ...
                    input_tensors = [model.metaval_total_loss1, model.metaval_total_losses2, model.summ_op]
            else:
